document.addEventListener("DOMContentLoaded", function() {
    ...
    marketItems.forEach(item => {
        ...
        if (listingsContainer) listingsContainer.appendChild(listingElement);
    });
    ...
    if (openButton) openButton.onclick = function() { modal.style.display = 'block'; }
    if (closeButton) closeButton.onclick = function() { modal.style.display = 'none'; }
    window.onclick = function(event) { if (event.target == modal) modal.style.display = 'none'; }

    // --- LÓGICA DO ENIGMA (AGORA SEGURA) ---
    ...
    // VERIFICAÇÃO DO ESTÁGIO 1 (RIDDLE)
    async function verificarRiddle() {
        const respostaUsuario = riddleInput.value;
        riddleButton.disabled = true;
        try {
            const response = await fetch('/.netlify/functions/verificar-alberti', {
                method: 'POST',
                body: JSON.stringify({ stage: 1, answer: respostaUsuario })
            });
            if (response.ok) {
    ...
    // VERIFICAÇÃO DO ESTÁGIO 2 (FRASE DECIFRADA)
    async function verificarDecifrado() {
        const fraseUsuario = decryptedInput.value;
        decryptedButton.disabled = true;
        try {
            const response = await fetch('/.netlify/functions/verificar-alberti', {
                method: 'POST',
                body: JSON.stringify({ stage: 2, answer: fraseUsuario })
            });
            if (response.ok) {
                const data = await response.json();
                if (data.success) {
                    // Constrói a recompensa dinamicamente com os dados do servidor
                    finalRewardDiv.innerHTML = `
                        <h2>TRANSMISSÃO RECEBIDA - FRAGMENTO 02</h2>
                        <img src="${data.reward.image}" alt="Fragmento de Mapa" class="ascii-art">
                        <p>Minha Ideologia:</p>
                        <div class="code-block">${data.reward.ideologia}</div>
                    `;
            ...
            decryptedButton.disabled = false;
        } catch (error) {
            alert("Erro de comunicação com o servidor de autenticação.");
            decryptedButton.disabled = false;
        }
    ...
    if(riddleButton) riddleButton.addEventListener('click', verificarRiddle);
    if(riddleInput) riddleInput.addEventListener('keyup', function(event) { if (event.key === "Enter") verificarRiddle(); });
    if(decryptedButton) decryptedButton.addEventListener('click', verificarDecifrado);
    if(decryptedInput) decryptedInput.addEventListener('keyup', function(event) { if (event.key === "Enter") verificarDecifrado(); });

    // --- LÓGICA DO DISCO DE ALBERTI ---
